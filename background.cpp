...
void BackGround::paintEvent(QPaintEvent*)
{
    ...
    for(int i = 0; i < 60; i++)
    {
        ...
        if(music_on)
        {
            ...
            {
                BrushColor.setRgb(PenColor.red()*0.75,PenColor.green()*0.75,PenColor.blue()*0.75);
            }
            else if((i + 60 - during) % 60 == 2 || (during + 60 - i) % 60 == 2)
            {
                BrushColor.setRgb(PenColor.red()*0.5,PenColor.green()*0.5,PenColor.blue()*0.5);
            }
            else if((i + 60 - during) % 60 == 3 || (during + 60 - i) % 60 == 3)
            {
                BrushColor.setRgb(PenColor.red()*0.25,PenColor.green()*0.25,PenColor.blue()*0.25);
            }
        }
